...
def get_slot_machine_spin(rows, cols, symbols):
    ...
    for symbol, symbol_count in symbols.items():
        for _ in range(symbol_count):
            all_synbols.append(symbol)

    columns = []
    for _ in range(cols):
        column = []
        current_symbols = all_synbols[:]
        for _ in range(rows):
...
def get_bet():
    while True:
        amount = input("What would you like to be on each lines? Rs.")
        if amount.isdigit():
            amount = int(amount)
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(f"You do not have enough bet that amount, your current balance is Rs.{balance}.")
        else:
            break
